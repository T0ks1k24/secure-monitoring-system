...
class ZoneService:
    ...
    async def create(self, data):
        ...
        if result:
            try:
                await rabbitmq_client.publish_zone_update(result.camera_id)
            except Exception:
                logger.exception("Failed to publish zone update after create for camera %s", result.camera_id)
        return result
    ...
    async def update(self, zone_id: int, data):
        ...
        if result:
            try:
                ...
                logger.exception("Failed to publish zone update after update for camera %s", result.camera_id)
        return result
    ...
    async def delete(self, zone_id: int):
        ...
        if camera_id:
            try:
                await rabbitmq_client.publish_zone_update(camera_id)
            except Exception:
                logger.exception("Failed to publish zone update after delete for camera %s", camera_id)
            return True
        return False
